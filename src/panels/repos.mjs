// Repos panel — branch hygiene and policy coverage across every repo at once.
//
...
export const title = "Repos & branch policy";
...
export function collect({ config, manifest, settings = {}, nowIso }) {
  ...
  const engineDirs = new Map(
    manifest.engines.map((eng) => [String(eng.dir).split(/[\\/]/)[0].toLowerCase(), eng]),
  );
  const engineRows = [];
  ...
  const unreportedEngines = manifest.engines
    .filter((eng) => !reported.has(String(eng.dir).split(/[\\/]/)[0].toLowerCase()))
    .map((eng) => eng.name);
